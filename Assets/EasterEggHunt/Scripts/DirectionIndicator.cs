...
using System.Collections.Generic;
    ...
    public class DirectionIndicator : Singleton<DirectionIndicator>
    {
        ...
        public bool DirectionIndicatorHelpOn = false;
        ...
        GameObject egg = EggCollectionManager.Instance.FindClosestObject(Cursor.transform.position);
            if (egg == null)
            {
                directionIndicatorRenderer.enabled = false;
                return;
            }
            Vector3 camToObjectDirection = egg.transform.position - Camera.main.transform.position;
            camToObjectDirection.Normalize();

            // The cursor indicator should only be visible if the target is not visible.
            isDirectionIndicatorVisible = !IsTargetVisible(egg);
        ...
        private bool IsTargetVisible(GameObject egg)
        {
            // This will return true if the target's mesh is within the Main Camera's view frustums.
            Vector3 targetViewportPosition = Camera.main.WorldToViewportPoint(egg.transform.position);
